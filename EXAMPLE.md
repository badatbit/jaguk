...
- 원장 수정(상자 이동·번역·스타일·그룹 이름…)은 wasm FS 의 원장에 쓰이고
  브라우저 **localStorage 로만** 미러된다 — 저장 시각·빌드 버전을 담은
  봉투(`{version, savedAt, text}`) 형식이라 언제·어느 버전에서 저장했는지
  남는다. 서버·디스크 기록은 없다.
- 읽어온 파일(원장·글꼴·이미지)은 **빌드 버전을 키로 브라우저 Cache
  Storage 에** 저장된다 — 재방문 시 재다운로드가 없고, 새 버전을 배포하면
  옛 캐시는 자동 삭제된다. typelet 소스는 캐시하지 않고 매번 재검증한다
  (커밋만으로 바뀌므로).
- 진입 페이지: `?stay=1` 저장 상태(수정 저장 시각·파일 캐시 버전) 보기 ·
  `?reset=1` 원장 수정 비우기 · `?purge=1` 파일 캐시 비우기.
